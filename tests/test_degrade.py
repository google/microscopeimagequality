...
degrader = quality.degrade.ImageDegrader(random_seed=0, photoelectron_factor=65535.0, sensor_offset_in_photoelectrons=0.0)
...
def test_set_exposure_with_offset_golden():
    ...
    degrader = quality.degrade.ImageDegrader(random_seed=0, photoelectron_factor=65535.0, sensor_offset_in_photoelectrons=100.0)
...
def test_evaluate_airy_psf_at_point():
    ...
    psf_value = quality.degrade.get_airy_psf(3, 3e-6, 0.0, 500e-9, 0.5, 1.0, False)[0, 1]
...
def test_degrade_images():
    ...
    quality.degrade.degrade_images(glob, output_path, 20e-6, 1.0, 0, 65535, 0, psf_width_pixels=21, pixel_size_meters=5e-6 / 21)

    degraded_image = quality.dataset_creation.read_16_bit_greyscale(os.path.join(output_path, "cell_image.png"))
...
def test_degrade_images_no_change():
    ...
    quality.degrade.degrade_images(glob, output_path, 0e-6, 1.0, 0, 65535, 0, psf_width_pixels=21, pixel_size_meters=40e-6 / 21, skip_apply_poisson_noise=True)
