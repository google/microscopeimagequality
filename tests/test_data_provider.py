...
class DataProviderTest(unittest.TestCase):
    ...
    def GetTFSession(self, graph):
        sv = tensorflow.train.Supervisor(logdir=os.path.join(self.test_dir, 'tmp_logs/'))
    ...
    def GetDataFromTfrecord(self):
        ...
        split_name = 'train'
        num_records = quality.data_provider.get_num_records(self.tfrecord_file_pattern %
                                                            split_name)
        self.assertEquals(TFRECORD_NUM_ENTRIES, num_records)
        dataset = quality.data_provider.get_split(
            split_name,
        ...
        [image, label, image_path] = provider.get([
            quality.data_provider.FEATURE_IMAGE, quality.data_provider.FEATURE_IMAGE_CLASS,
            quality.data_provider.FEATURE_IMAGE_PATH
        ])
    ...
    def testGetSplit(self):
        ...
        with g.as_default():
            ...
            self.assertListEqual(
                list(np_image.shape),
                [quality.data_provider.IMAGE_WIDTH, quality.data_provider.IMAGE_WIDTH, 1])
    ...
    def testBatching(self):
        ...
        with g.as_default():
            ...
            images, labels, image_paths = quality.data_provider.get_batches(
                expanded_image,
                expanded_label,
                expanded_image_path,
            ...
            self.assertListEqual(
                list(np_images.shape), [
                    self.batch_size, quality.data_provider.IMAGE_WIDTH,
                    quality.data_provider.IMAGE_WIDTH, 1
                ])
    ...
    def testGetImagePatchTensor(self):
        ...
        with g.as_default():
            image, label, image_path = self.GetDataFromTfrecord()
            patch, label, image_path = quality.data_provider.get_image_patch_tensor(
    ...
    def testApplyRandomBrightnessAdjust(self):
        ...
        with g.as_default():
            ...
            factor = 2.0
            patch = quality.data_provider.apply_random_brightness_adjust(image, factor,
                                                                         factor)
    ...
    def testGetImageTilesTensor(self):
        ...
        with g.as_default():
            image, label, image_path = self.GetDataFromTfrecord()
            tiles, labels, image_paths = quality.data_provider.get_image_tiles_tensor(
                image, label, image_path, patch_width=patch_width)
    ...
    def testProvideDataImagePath(self):
        g = tensorflow.Graph()
        with g.as_default():
            _, _, image_paths, _ = quality.data_provider.provide_data(
                self.tfrecord_file_pattern,
                split_name='train',
    ...
    def testProvideDataWithDeterministicOrdering(self):
        ...
        with g.as_default():
            ...
            for i in range(num_batches_tested):
                ...
                self.assertTrue(numpy.all(image_class[0] == image_class))
                self.assertTrue(numpy.all(np_image_paths[0] == np_image_paths))
