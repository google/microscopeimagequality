...
,"testdata")
  ...
  def testGetAiryPsfGolden(self):
    psf = degrade.get_airy_psf(20, 5e-6, 4.0e-6, 500e-9, 0.5, 1.0)
    expected_psf = self.get_test_image("psf.png")
    np.testing.assert_almost_equal(expected_psf, psf, 5)

  def testWritePngRuns(self):
    psf = self.get_test_image("psf.png")
    degrade.write_png(psf, os.path.join(self.test_data_directory, "psf.png"))

  def testReadWritePng(self):
    image = self.get_test_image("cell_image.tiff")
    output_path = os.path.join(self.test_data_directory, "cell_image2.png")
  ...
  def testDegradeImages(self):
    ...
    output_path = self.test_data_directory
