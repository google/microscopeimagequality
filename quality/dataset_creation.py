...
def convert_to_examples(dataset,
  ...
  """
    ...
    with open(
            data_provider.get_filename_num_records(
...
def generate_tf_example(image, label, image_path):
  ...
  """
    ...
    features.feature[data_provider.FEATURE_IMAGE_CLASS].float_list.value.extend(
        (label.flatten().tolist()))

    features.feature[data_provider.FEATURE_IMAGE_PATH].bytes_list.value.append(
        image_path)
